...
type FileSearcher(pattern : string,  
    ...
    let regex = 
        try
            match (pattern, caseSensitive, simpleMatch) with 
    ...
    let accumError msg =
        ...
        let r = Regex("")
        bc.Add(LineMatch("Error:" + msg , "", 0, "", (r.Match(""))))
    ...
    let excludeRegexes =
        ...
        let rec loop regexs xs =
            ...
            | hd::tl ->
                try 
                    loop (Regex(hd)::regexs) tl
                with _ ->
                    accumError ("Malformed regular expression in exclude regex " + hd)
    ...
    let GetIncludedFiles dir (excludeRegexes :  Regex list) =  
        ...
        seq { 
            for patt in includePatterns do 
                yield! Directory.EnumerateFiles(dir, patt) 
        } 
        |> Seq.filter(fun s ->
            excludeRegexes
            |> List.exists(fun x -> x.IsMatch(s)) 
            |> not
            )
    ...
    let (|Contents|AccessDenied|) (dir, (excludeRegexes :  Regex list)) =  
            try 
                Success (GetIncludedFiles dir excludeRegexes,  
    ...
    let rec GetFiles (excludeRegexes :  Regex list) dir  =  
        seq { 
            match (dir, excludeRegexes) with 
                | Contents(files, directories) -> 
                    yield! files 
                    yield! directories 
                            |> Seq.collect (GetFiles excludeRegexes)
    ...
    let CollectLineMatches (file : string)  (regex : Regex) = 
        try 
            if not IncludeExecute && ( file.ToLower().EndsWith(".exe") || file.ToLower().EndsWith(".dll")) then ()
            else
